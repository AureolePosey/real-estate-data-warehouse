...
def create_star_schema():
    ...
    # Create dim_location
    dim_location = df_curated.select("city", "state", "zip_code").distinct().withColumn("location_id", F.monotonically_increasing_id())
    ...
    # Create dim_property
    dim_property = df_curated.select("status", "bed", "bath", "acre_lot","house_size", "property_size_category").distinct().withColumn("property_id", F.monotonically_increasing_id())
    ...
    # Create dim_date 
    dim_date = df_curated.select("sale_date","sale_year","sale_month","sale_quarter").distinct().withColumn("date_id", F.monotonically_increasing_id())
    ...
    # Create fact_sales
    #first join with dim_location to get location_id in fact_sales
    fact_sales = df_curated.join(dim_location, on=["city", "state", "zip_code"], how="left") 

    #second join with dim_property
    fact_sales = fact_sales.join(dim_property, on=["status", "bed", "bath", "acre_lot","house_size", "property_size_category"], how="left")
    ...
    fact_sales = fact_sales.select("location_id", "property_id", "date_id", "price", "price_per_sqft")
